...
class DQN_Solver:
    ...
    def choose_action(self, observation):
        ...
        if random.random() < eps_threshold:
            return np.random.choice(np.array(range(9)), p=[0.075,0.3,0.075,0.05,0,0.05,0.075,0.3,0.075])    # sample random action with set priors (if we flap too much we will die too much at the start and learning will take forever)
